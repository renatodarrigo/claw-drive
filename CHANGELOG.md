...
- **Decision-context prose no longer implies `rationale` is always present.** The `poll_turn`/`poll_session` tool descriptions and the README/reference/driving-patterns decision-context prose now match `events.ts` (optional by contract): `rationale` derives from B's preceding assistant text and is absent when B goes straight to the call.
- **The stale-settle cost gap is now documented.** A checkpoint distill that settles only after its result is unusable (the session exited, a stop or teardown began, or the checkpoint block changed mid-flight) is discarded with its already-incurred cost unmetered — design-accepted fail-open, previously stated nowhere in the budget docs.
...
- **A policy's `budget` block rejects an unrecognized key instead of silently ignoring it.** A typo'd cap name — `max_cost_us` instead of `max_cost_usd`, say — is caught as an unrecognized key rather than accepted as a silent no-op, so the safety cap its author configured actually takes effect. `validatePolicy` checks every key inside `budget` against the four known caps (`max_tool_calls`, `max_wall_clock_seconds`, `max_consecutive_errors`, `max_cost_usd`) and names any other one in its error, the same convention already used for an unknown top-level or `rotation` key. An empty `budget: {}`, and an underscore-prefixed key inside it (e.g. a carried-over `_comment`), are both accepted.
- **`validatePolicy` rejects a rule's malformed `arg_matches`, instead of passing it through unchecked.** A non-object shape, a non-string value, or a pattern that doesn't compile (e.g. `{ file_path: "(" }`) is caught up front, naming the list, rule index, and key — the same format `bash_command_matches` already uses, rather than surfacing later as a thrown exception at match time or a separate `policy lint` finding.
- **`send_turn` and `provide_tool_output` refuse a session whose process has already exited, instead of starting a turn nothing will ever run.** A driven session's process can crash, get stopped, or receive a signal while its runner keeps the control socket open for a bounded window — crash distillation alone can hold it open up to 180s — and a call landing in that window is refused outright rather than emitting a phantom `turn_started` and writing to the closed stdin. Both ops return `SESSION_EXITED` (`session process has exited; turn cannot start — use recover`), mirroring `rotate`'s own dead-B refusal; a live session's turns are unaffected.
- **A rotation aborted by a death or a stop landing anywhere during the second handover attempt — its own turn, or its own post-interrupt settle window after a timeout — names the truthful cause, instead of a generic one.** If the session process dies, or a stop/circuit-breaker teardown engages, at any point during that second attempt, `rotation_failed` reports `b_exited:` (pointing at `recover`) or `session_stopping:` — matching the rotation's other two abort points — rather than the generic `handover_generation_failed: no extractable <handover> block after 2 attempts`. A genuinely wedged turn (B alive, never acknowledging the interrupt) and a genuine both-attempts exhaustion with B alive keep their existing reasons.
