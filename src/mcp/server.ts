...
export async function runMcpServer(): Promise<void> {
  ...
  const handlers: Record<string, (args: Record<string, unknown>) => Promise<HandlerResult>> = {
    start_session: handleStartSession,
    stop_session: handleStopSession,
    send_turn: handleSendTurn,
    poll_turn: handlePollTurn,
    poll_session: handlePollSession,
    list_sessions: handleListSessions,
    resolve_tool_call: handleResolveToolCall,
  ...
  const tools = MCP_TOOL_DEFS.map((d) => {
    ...
    if (!handler) throw new Error(`no handler wired for MCP tool: ${d.name}`);
    return { ...d, handler };
  });
