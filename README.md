...
Several drivers can use claw-drive on one machine at once — two Claude Code sessions each supervising their own B's — and they share one sessions root. Every session therefore carries a **fleet tag**, and every listing acts as one fleet:
...
| `prune [--older-than 24h] [--force] [--fleet TAG] [--all-fleets]` | Remove dead sessions in the fleet view older than cutoff. `--force` also removes a dead session whose crash-handover hasn't been consumed yet. |
